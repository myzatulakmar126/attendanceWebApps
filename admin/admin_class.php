...
Class Action {
	...
	function upload_receipt(){
		...
		if($_FILES['receipt']['tmp_name'] != ''){
			$fname = strtotime(date('y-m-d H:i')).'_'.$_FILES['receipt']['name'];
			$move = move_uploaded_file($_FILES['receipt']['tmp_name'],'../assets/receipt/'. $fname);
			if($move){
				$save = $this->db->query("UPDATE orders SET receipt_path = '" . $fname . "' WHERE id =". $id);
				echo '<script>console.log("masuk failed", '. json_encode($move) .')</script>';
			} else{
				echo '<script>console.log("masuk berjaya", '. json_encode($move) .')</script>';
			}
		}
		return 1;
	...
	function delete_cart(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM cart WHERE product_id = ". $id ." AND user_id = " . $_SESSION['login_user_id']);
		if($delete)
			return 1;
	}
}